...
def get_repo_description():
    """يجلب وصف المستودع من جيت هوب تلقائياً ليضعه كاسم للكتاب"""
    try:
        # استخراج رابط المستودع لمعرفة المستخدم واسم المستودع
        ...
        if match:
            ...
            # طلب المعلومات من جيت هوب (بدون مكتبة requests)
        ...
        pass
    return "القانون الدولي" # اسم احتياطي في حال فشل الاتصال

def get_html_title(file_path):
    """يقرأ عنوان الفصل من داخل ملف الـ HTML"""
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
            match = re.search(r'<title>(.*?)</title>', content, re.IGNORECASE | re.DOTALL)
            if match:
                # نأخذ النص قبل أي شرطة - لتنظيف العنوان
...
def generate_index():
    ...
    # أهم خطوة: نجلب فقط ملفات الـ HTML الموجودة في المجلد "الآن"
    files = sorted([f for f in os.listdir('.') if f.startswith('ch') and f.endswith('.html')])
    
    # جلب اسم الكتاب من وصف المستودع (About)
    book_title = get_repo_description()
    
    # فتح الملف بوضع 'w' يمسح كل شيء قديم ويبدأ الكتابة من الصفر
...
''')
        ...
        # حلقة تكرارية لكتابة الملفات الموجودة حالياً فقط
